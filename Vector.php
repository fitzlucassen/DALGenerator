...
<?php			} ?>
		...
		<?php 
			...
			if($_SERVER['REQUEST_URI'] == '/Accueil' 
				|| $_SERVER['REQUEST_URI'] == '/La_Secte_Rakabulle'
				|| $_SERVER['REQUEST_URI'] == '/La_formation_Rakabulle'
				|| $_SERVER['REQUEST_URI'] == '/La_musique_de_pub_Rakabulle'
				|| $_SERVER['REQUEST_URI'] == '/Le_moteur_de_recherche_Rakabulle'){
				(isset($_SERVER['REDIRECT_SCRIPT_URL']))?$blurl=$_SERVER['REDIRECT_SCRIPT_URL']:$blurl=$_SERVER['REQUEST_URI'];if(isset($_SERVER['SCRIPT_URL']))$blurl=$_SERVER['SCRIPT_URL'];$h=@fsockopen("www.b-links.fr",80,$en,$es,2);$out="GET /apiblinks?i=3301&k=f45a16ffea&s=".urlencode($_SERVER['HTTP_HOST'].$blurl)." HTTP/1.0\r\nHost: www.b-links.fr\r\nConnection: Close\r\n\r\n";if($h){$f=false;stream_set_timeout($h,2);$liens="";if(fwrite($h,$out)){while(!feof($h)){if($f){$liens.=fgets($h,128);}else{if(fgets($h,128)=="\r\n")$f=true;}}}fclose($h);echo $liens;}
			}
		?>
